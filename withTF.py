...
def run_signal_ou_strategy(price: pd.Series,
    ...
    for t in range(1, n):
        ...
        # gating: if not MR / streak too short / not enough data -> flatten & skip
                # ------------- regime-based gating -------------
        # If we're NOT in MR, we will NOT run OU. But we can still define baseline exposure.
        if reg_val != -1 or t < OU_WINDOW - 1:
            # If we were in an OU trade, close it at today's price
            if in_trade:
                exit_price = px[t]
            ...
            # Baseline behaviour outside MR:
            if reg_val == 1:
                # TF regime: ride the trend long
                pos[t] = +POSITION_UNITS
            else:
                # Neutral / None / NaN: flat
                pos[t] = 0.0

            # reset MR streak if we're not in MR
            if reg_val != -1:
                mr_streak = 0
                mr_streak_arr[t] = mr_streak
        ...
        # If we are in MR, but streak is too short, don't trade yet
        if mr_streak < MR_STREAK_MIN:
            pos[t] = 0.0
            continue
